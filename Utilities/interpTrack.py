...
def interpolateTrack(configFile, trackFile, source, delta=0.1,
    ...
    if len(track.Indicator) <= 2:
        # Use linear interpolation only (only a start and end point given):
        nLon = scint.interp1d(timestep, track.Longitude, kind='linear')(newtime)
        nLat = scint.interp1d(timestep, track.Latitude, kind='linear')(newtime)
        npCentre = scint.interp1d(timestep, track.CentralPressure, kind='linear')(newtime)
        npEnv = scint.interp1d(timestep, track.EnvPressure, kind='linear')(newtime)
        nrMax = scint.interp1d(timestep, track.rMax, kind='linear')(newtime)
        ...
        if interpolation_type == 'akima':
            ...
            try:
                ...
                nLon = scint.splev(newtime, scint.splrep(timestep, track.Longitude, s=0), der=0)
                nLat = scint.splev(newtime, scint.splrep(timestep, track.Latitude, s=0), der=0)
            else:
